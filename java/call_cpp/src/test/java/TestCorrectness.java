...
public class TestCorrectness
{
	...
	private static MetaFFIModule cppModule;
	...
	public static void setUp()
	{
		...
		String modulePath = sourceRoot.replace('\\', '/') +
			"/sdk/test_modules/guest_modules/cpp/test_bin/" + getCppGuestModuleFilename();
		cppModule = runtime.loadModule(modulePath);
		assertNotNull("Failed to load C++ guest module", cppModule);
	}

	private static String getCppGuestModuleFilename()
	{
	...
	{
		Caller c = cppModule.load(entity, params, retvals);
	...
	{
		Caller fn = load("callable=xcall_no_op", null, null);
		fn.call();
	}
	...
	{
		Caller fn = load("callable=xcall_hello_world", null,
	...
	{
		Caller fn = load("callable=xcall_returns_an_error", null, null);
		try
		{
			fn.call();
			fail("xcall_returns_an_error did not throw");
		}
		catch (Throwable e)
		{
			// Expected: C++ exception propagated as error
		}
	}
	...
	{
		Caller fn = load("callable=xcall_div_integers",
	...
	{
		Caller getCtr = load("callable=xcall_get_counter", null,
			new MetaFFITypeInfo[]{t(MetaFFITypes.MetaFFIInt64)});
		Caller setCtr = load("callable=xcall_set_counter",
			new MetaFFITypeInfo[]{t(MetaFFITypes.MetaFFIInt64)}, null);
		Caller incCtr = load("callable=xcall_inc_counter",
			new MetaFFITypeInfo[]{t(MetaFFITypes.MetaFFIInt64)},
			new MetaFFITypeInfo[]{t(MetaFFITypes.MetaFFIInt64)});
